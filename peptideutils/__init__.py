...
peptideutils_peptoid_letters = 'Na', 'Nab', 'Nd', 'Ne', 'Nf', 'Nfe', 'NfeBr', 'NfeCl', "Nfex", 'Nfn', 'Nfnap', 'Nfp', 'Ni', 'Nk', 'Nke', 'Nkeqm', 'Nl', 'Nm', 'NmO', 'Nn', 'Nq', 'Nr', 'Ns', 'Nse', 'Nt', 'Nv', 'Nw', 'Nwe', 'Ny'
...
def index2peptoid(index, Length):
    ...
    step = int(size/29)
    above_size = int(size/20)
    steps = np.array([i*step for i in range(29)])
    solution = []
    letter_i = np.where(steps <= index)[0][-1]
    letter = letters_1[letter_i]
    solution.append(letter)
    while len(solution) < Length:
        ...
        step = int(step/29)
        steps = np.array([i*step for i in range(29)])
        if index < above_size/29 and len(solution) < Length-1:
            letter = "A"
            #print(index <= above_size/20)
        else:
            letter_i = np.where(steps <= index)[0][-1]
